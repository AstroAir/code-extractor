...
# ---------------------------------------------------------------------------
# Advanced error handling classes (merged from advanced_error_handling.py)
# ---------------------------------------------------------------------------
...
class AdvancedErrorCollector:
    ...
    async def add_error(
        ...
        async with self._lock:
            ...
            # Log error
            log_level = {
                ErrorSeverity.DEBUG: _logger.debug,
                ErrorSeverity.INFO: _logger.info,
                ErrorSeverity.WARNING: _logger.warning,
                ErrorSeverity.ERROR: _logger.error,
                ErrorSeverity.CRITICAL: _logger.critical,
            }[severity]
...
class CircuitBreaker:
            ...
            try:
                ...
                if self.state == "half_open":
                    ...
                    if self.success_count >= self.success_threshold:
                        self.state = "closed"
                        self.failure_count = 0
                        _logger.info("Circuit breaker closed - service recovered")
                ...
                if self.failure_count >= self.failure_threshold:
                    self.state = "open"
                    _logger.warning(f"Circuit breaker opened due to {self.failure_count} failures")
    ...
    def can_execute(self) -> bool:
        ...
        Useful for manual circuit breaker control when wrapping with
        :meth:`call` is not practical (e.g., async generators).
        """
    ...
    def record_success(self) -> None:
        """Record a successful operation for manual circuit breaker control."""
        if self.state == "half_open":
            self.success_count += 1
            if self.success_count >= self.success_threshold:
                self.state = "closed"
    ...
    def record_failure(self) -> None:
        """Record a failed operation for manual circuit breaker control."""
        self.failure_count += 1
        self.last_failure_time = time.time()
        if self.failure_count >= self.failure_threshold:
            self.state = "open"
...
class RecoveryManager:
    ...
    async def attempt_recovery(
        ...
        if attempts >= self.max_recovery_attempts:
            _logger.warning(f"Max recovery attempts exceeded for {attempt_key}")
            return False
        ...
        if not strategy:
            _logger.debug(f"No recovery strategy for category {error.category}")
            return False

        try:
            _logger.info(f"Attempting recovery for {error.error_id} (attempt {attempts + 1})")
            ...
            if success:
                ...
                _logger.info(f"Recovery successful for {error.error_id}")
            else:
                error.recovery_attempted = True
                error.recovery_successful = False
                self.failed_recoveries += 1
                _logger.warning(f"Recovery failed for {error.error_id}")

            return success

        except Exception as e:
            _logger.error(f"Recovery strategy failed for {error.error_id}: {e}")
            error.recovery_attempted = True
            error.recovery_successful = False
            self.failed_recoveries += 1
            return False
    ...
    async def _recover_file_access(
        ...
        try:
            ...
            if not Path(file_path).exists():
                _logger.debug(f"File {file_path} no longer exists, skipping")
                return True  # Consider this a successful recovery

            # Check permissions
            if not Path(file_path).is_file():
                _logger.debug(f"Path {file_path} is not a file, skipping")
                return True
            ...
            for encoding in encodings:
                try:
                    ...
                    _logger.info(f"File {file_path} readable with encoding {encoding}")
                    return True
                except UnicodeDecodeError:
                    continue

            _logger.warning(f"File {file_path} not readable with any encoding")
            return False

        except Exception as e:
            _logger.error(f"File access recovery failed: {e}")
            return False
    ...
    async def _recover_network(
        ...
        _logger.info(f"Network recovery: waiting {backoff_time}s before retry")
    ...
    async def _recover_memory(
        ...
        if "batch_size" in context:
            ...
            _logger.info(f"Reduced batch size to {new_batch_size} for memory recovery")
            return True

        _logger.warning("Cannot recover from memory error - no batch size to reduce")
        return False
    ...
    async def _recover_timeout(
        ...
        if "timeout" in context:
            ...
            _logger.info(f"Increased timeout to {new_timeout}s for recovery")
            return True
    ...
    async def _recover_parsing(
        ...
        _logger.info(f"Skipping problematic content in {error.context}")
        return True
    ...
    async def _recover_dependency(
        ...
        _logger.info(f"Using fallback for missing dependency in {error.context}")
        return True
...
class EnhancedErrorHandler:
    ...
    def __init__(self, config: Any) -> None:
        self.config = config
        self.error_collector = AdvancedErrorCollector()
    ...
    async def _log_error_to_file(
        ...
        try:
            ...
            _logger.error(f"Failed to log error to file: {e}")
