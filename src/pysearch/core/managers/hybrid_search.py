"""
Hybrid search capabilities including semantic and multi-strategy search.
...
Classes:
    HybridSearchManager: Manages hybrid search capabilities
...
Key Features:
    ...
    - Result filtering and clustering

Example:
    Using hybrid search:
        >>> from pysearch.core.managers.hybrid_search import HybridSearchManager
...
class HybridSearchManager:
    """Manages hybrid search capabilities including semantic and multi-strategy search."""

    def __init__(self, config: SearchConfig) -> None:
        self.config = config
        self.semantic_engine: Any = None
    ...
    def search_semantic(
        ...
        **kwargs: Any,
        ...
        if (
            ...
            and not self.semantic_engine.embedding_model.is_fitted
            and file_paths
        ):
            ...
            try:
                ...
                if metadata_filters:
                    ...
                    if file_metadata and not apply_metadata_filters(
                        file_metadata, metadata_filters
                    ):
                        continue
        ...
        if self._logger:
            self._logger.log_search_complete(
                pattern=f"semantic:{query}", results_count=len(search_items), elapsed_ms=elapsed_ms
            )
    ...
    async def hybrid_search(
        ...
        metadata_index_search_func: Callable | None = None,
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            metadata_index_search_func: Function for metadata index search (optional)
        ...
        results: dict[str, Any] = {
            ...
            "graphrag": None,
            "metadata_index": None,
            "metadata": {"pattern": pattern, "timestamp": time.time(), "methods_used": []},
        }
        ...
        if use_graphrag and graphrag_search_func:
            try:
                ...
                graphrag_query = GraphRAGQuery(
                    pattern=pattern, max_hops=graphrag_max_hops, include_relationships=True
                )
        ...
        if use_metadata_index and metadata_index_search_func:
            try:
                from ...indexing.metadata import IndexQuery

                index_query = IndexQuery(semantic_query=pattern, include_entities=True, limit=50)
                index_results = await metadata_index_search_func(index_query)
                if index_results:
                    results["metadata_index"] = index_results
                    results["metadata"]["methods_used"].append("metadata_index")
            except Exception as e:
                if self._logger:
    ...
    def cluster_results_by_similarity(
        self, search_items: list, similarity_threshold: float = 0.8
    ) -> list:
        """
    ...
    def rank_results(self, search_items: list, pattern: str, strategy: str = "hybrid") -> list:
        ...
        """
        try:
            from ...search.scorer import RankingStrategy, sort_items
