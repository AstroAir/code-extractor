...
from pysearch import Language, MetadataFilters, OutputFormat, PySearch, SearchConfig
...
def test_metadata_filters_size_language_and_regex_error(tmp_path: Path) -> None:
    ...
    res_lang = eng.search(
        "print", regex=False, context=0, output=OutputFormat.TEXT, metadata_filters=md_lang
    )
    ...
    res_size = eng.search(
        "print", regex=False, context=0, output=OutputFormat.TEXT, metadata_filters=md_size
    )
