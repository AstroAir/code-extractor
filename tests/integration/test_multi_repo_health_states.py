...
def test_repository_info_health_states(tmp_path: Path) -> None:
    ...
    repo_missing = RepositoryInfo(
        name="missing", path=missing_path, config=SearchConfig(paths=[str(missing_path)])
    )
    ...
    repo_no_git = RepositoryInfo(
        name="no_git", path=no_git_path, config=SearchConfig(paths=[str(no_git_path)])
    )
...
def test_repository_manager_error_logging(tmp_path: Path) -> None:
    ...
    assert (
        rm.add_repository("test", valid_path, config=SearchConfig(paths=[str(valid_path)])) is True
    )
    assert (
        rm.add_repository("test", valid_path, config=SearchConfig(paths=[str(valid_path)])) is False
    )

    # Test adding repository with non-existent path (should log error and return False)
    invalid_path = tmp_path / "invalid"
    assert (
        rm.add_repository("invalid", invalid_path, config=SearchConfig(paths=[str(invalid_path)]))
        is False
    )
