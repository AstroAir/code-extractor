...
pip install git+https://github.com/AstroAir/pysearch.git@v1.0.0
...
pysearch provides several optional dependency groups for advanced features:

```bash
# File watching support (real-time file monitoring)
pip install -e ".[watch]"
# Includes: watchdog, psutil

# GraphRAG and enhanced analysis
pip install -e ".[graphrag]"
# Includes: qdrant-client, numpy, sentence-transformers, scikit-learn

# Vector database support
pip install -e ".[vector]"
# Includes: qdrant-client, numpy, faiss-cpu

# Advanced semantic search (transformer models)
pip install -e ".[semantic]"
# Includes: sentence-transformers, transformers, torch, scikit-learn

# All optional features combined
pip install -e ".[all]"
```

| Dependency Group | Features Enabled |
|------------------|-----------------|
| `watch` | Real-time file monitoring via `FileWatcher` and `WatchManager` |
| `graphrag` | GraphRAG engine, knowledge graphs, Qdrant integration |
| `vector` | Vector database backends (LanceDB, Qdrant, Chroma), FAISS |
| `semantic` | Transformer-based embeddings, advanced semantic search |
...
1. **Read the Usage Guide**: [Usage Guide](../guide/usage.md)
2. **Configure pysearch**: [Configuration Guide](../guide/configuration.md)
...
1. **Check the FAQ**: [FAQ](../help/faq.md)
