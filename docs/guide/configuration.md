...
Configure vector database backends for semantic and GraphRAG search.

### LanceDB (Default, Local)

```python
from pysearch.storage import LanceDBProvider

provider = LanceDBProvider(
    db_path="./pysearch-vectors",  # Local storage path
...
```python
from pysearch.storage import QdrantProvider, QdrantConfig
...
```python
from pysearch.storage import ChromaProvider
...
```python
from pysearch.storage import EmbeddingConfig
...
### TOML Configuration for Vector DB

```toml
# pysearch.toml
[vector_db]
provider = "lancedb"        # lancedb, qdrant, or chroma
db_path = "./pysearch-vectors"
...
Configure GraphRAG (Graph Retrieval-Augmented Generation) for knowledge graph-based search.
...
graph = builder.build_from_codebase(
    "./src",
    entity_types=["function", "class", "method", "variable", "import"],
    max_depth=5
)
...
extractor = EntityExtractor(
    include_private=False,     # Skip _private entities
    include_dunder=False,      # Skip __dunder__ methods
    extract_docstrings=True,   # Include docstrings as metadata
    max_file_size=1_000_000    # Skip large files
)
```

### TOML Configuration for GraphRAG

```toml
# pysearch.toml
[graphrag]
enabled = true
entity_types = ["function", "class", "method", "import"]
...
- [Architecture](../advanced/architecture.md) - Internal design and components
