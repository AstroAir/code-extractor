...
- [Search Types](#search-types) (Text, Regex, AST, Semantic, Fuzzy, Boolean, GraphRAG)
- [Output Formats](#output-formats)
- [Advanced Features](#advanced-features) (Caching, File Watching, Multi-Repo, Dependency Analysis, IDE Integration, Distributed Indexing)
...
pysearch find --pattern "authetication" --fuzzy --threshold 0.7

# Boolean search (logical operators)
pysearch find --pattern "(async AND handler) NOT test" --logic

# GraphRAG search (knowledge graph)
pysearch graphrag --query "authentication handler" --path ./src
```
...
Conceptual search using lightweight semantic analysis. Optionally supports transformer-based embeddings for higher accuracy.
...
Typo-tolerant matching using multiple algorithms (Levenshtein, Jaro-Winkler, n-gram).
...
results = fuzzy_search_advanced(
    query="authetication",
    candidates=["authentication", "authorization"],
...
- Finding code despite typos in queries
- Approximate name matching
- Spelling correction suggestions

### Boolean Search

Logical query composition with `AND`, `OR`, `NOT` operators.
...
- Excluding specific patterns from results
- Combining multiple search conditions

### GraphRAG Search

Knowledge graph-based search combining entity extraction, relationship mapping, and vector similarity.
...
results = engine.search("authentication handler")
```

**Use cases:**

- Understanding code architecture and relationships
- Finding related functions and classes across the codebase
- Exploring unfamiliar codebases via knowledge graphs
...
print(f"Modules: {metrics.total_modules}, Coupling: {metrics.avg_coupling:.2f}")

# Detect circular dependencies
detector = CircularDependencyDetector(graph)
for cycle in detector.find_cycles():
    print(f"Circular: {' -> '.join(cycle)}")
```

### IDE Integration

LSP-like features for jump-to-definition, find references, completions, and more.

```python
from pysearch.integrations import IDEHooks
...
Multi-process parallel indexing for large codebases.

```python
from pysearch.integrations import DistributedIndexingEngine

dist_engine = DistributedIndexingEngine(num_workers=4)
dist_engine.index_codebase("./large-monorepo")
```
...
1. **Check documentation:** See [API Reference](../api/index.md) for detailed information
...
- **API Reference:** Check [API Reference](../api/index.md) for detailed documentation
- **Architecture:** Learn about internals in [Architecture](../advanced/architecture.md)
